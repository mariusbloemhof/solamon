...
| `source` | string | `modbus_poll` (regular cycle), `replay_buffer` (drained from local SQLite after reconnect), or `mqtt_push` (reserved for AXM-WEB2-driven Acuvim L push, post-MVP). |
...
```json
{
  ...
  "properties": {
    ...
    "source": { "type": "string", "enum": ["modbus_poll", "replay_buffer", "mqtt_push"] },
...
|--------|-------|--------------|
| `acknowledged` | Pi received the command, validated against profile, has not yet executed. | Optional — most flows skip this and go straight to `confirmed`/`failed`. |
...
The `expired` state is set cloud-side only (the Pi never publishes `expired`; the cloud TTL reaper sets it).
...
Per-site ACL entries, configured in `mosquitto/acls`:
